    ...
    class Object: public Manager{
        ...
        public:
            ...
            bool init(){
                ...
                if(rc != SQLITE_OK){
                    fprintf(stderr, "SQL error init object: %s\ncmd :  %s\n", err_msg, cmd); 
            ...
            bool insert(models::BaseObject* obj){
                ...
                int rc = sqlite3_exec(db, cmd.c_str(),  callback, 0, 
                        &err_msg);
                if( rc != SQLITE_OK ){
                    fprintf(stderr, "SQL error insert obj: %s\ncmd : %s\n", err_msg, cmd.c_str());
                    sqlite3_free(err_msg);
                    return false;
                }
            ...
            static int init_obj(void* _objects, int argc, char **argv,
                ...
                if( i_attrs != ""){
                    for(size_t i=0; i<values_i.size(); ++i){
                        ...
                        (*objects_ptr)[name].set(tmp[0], std::stoi(tmp[1]), std::stof(tmp[2]));
                    }
                }
                if( str_attrs != ""){
                    for(size_t i=0; i<values_str.size(); ++i){
                        ...
                        (*objects_ptr)[name].set(tmp[0], tmp[1], std::stof(tmp[2]));
                    }
                }
                return 0;
            }
            ...
            bool init_objs(){
                ...
                if( rc !=SQLITE_OK ){
                    fprintf(stderr, "SQL error init objects: %s\n", err_msg);
                    sqlite3_free(err_msg);
                    return false;
                }else{
                    fprintf(stdout, "Operation init objects done successfully\n");
                }
                return true;
            }
    };
