...
vector<cluster_t*> get_centres(Matrix<d_cl, d_obj>& graph, agent_t& a, vector<cluster_t*>blobs, vector<cluster_t*>centres) {
...
VPII convert(Matrix<d_cl,d_obj> graph, uint64_t k) {
    VPII edges;
    uint64_t i, j;
...
vector<uint64_t> azka(Matrix<d_cl,d_obj> graph, agent_t& a, vector<uint64_t>& labels, vector<cluster_t*>& blobs) {
    ...
    for (i = 0; i < n_centres; ++i) {
	    ...
	    distances.push_back(Dijkstra(edges, centres[i]->get_id()));
    };
